...
$username=$name = $email = $city = $state = $amount = $contact ="";
$usernameErr=$nameErr = $emailErr = $cityErr = $stateErr  = $amountErr = $contactErr  ="";
 ...
 if($_SERVER["REQUEST_METHOD"] == "POST"){
                    ...
                    if (empty($_POST["contact"])) {  
                        ...
                        if (strlen ($contact) != 10) {  
                            $contactErr = "CONTACT-NO must contain 10 digits."; 
                            }  
                    }  
                ...
                if(empty($nameErr) && empty($emailErr) && empty($stateErr) && empty($cityErr) && empty($contactErr) && empty($amountErr) && empty($usernameErr)){
                    
                    // Prepare an insert statement
                    $sql = "INSERT INTO donation (username,name,email,contact,city,state,amount) VALUES ('$username','$name','$email','$contact','$city','$state','$amount')";
...
<body>
        ...
        <div class="one">
            <div class="col-md-8">
            <img src="login.jpeg" alt="Image" class="img-fluid" >
          </div>
 
                    
    <div class="wrapper">
       <font color="black"> <h4>DONATION BOX</h4>

        <p>Please fill in your details to DONATE.</p>
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
            
            <div class="form-group  <?php echo (!empty($nameErr)) ? 'has-error' : ''; ?>">
                <label>Name of A Donator</label>
           ...
           <div class="form-group  <?php echo (!empty($amountErr)) ? 'has-error' : ''; ?>">
                <label>Amount</label>
