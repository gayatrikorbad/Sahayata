...
$username=$name = $email = $city = $state = $education = $contact = $experience = $images="";
$usernameErr=$nameErr = $emailErr = $cityErr = $stateErr = $educationErr = $contactErr = $experienceErr = $imageErr = "";
             ...
             $username=$_SESSION['username'];
                    ...
                    if (empty($_POST["experience"])) {  
                        ...
                        $experience = input_data($_POST["experience"]);  
                            // check if name only contains letters and whitespace  
                            if (!preg_match("/^[a-zA-Z-]*$/",$experience)) {  
                                $experienceErr = "Only alphabets and white space are allowed";  
                    ...
                    if (empty($_POST["education"])) {  
                        ...
                        $education = input_data($_POST["education"]);  
                            // check if name only contains letters and whitespace  
                            if (!preg_match("/^[a-zA-Z ]*$/",$education)) {  
                                $educationErr = "Only alphabets and white space are allowed";  
                            }  
                    }  
                ...
                if(empty($nameErr) && empty($emailErr) && empty($stateErr) && empty($cityErr) && empty($contactErr) && empty($imageErr) && empty($experienceErr) && empty($educationErr) && empty($usernameErr)){
                    
                    // Prepare an insert statement
                    $sql = "INSERT INTO application (username,name,email,contact,education,city,state,image,experience) VALUES ('$username','$name','$email','$contact','$education','$city','$state','$images','$experience')";
...
<body>
        ...
        <div class="one">
          <div class="col-md-6">
            <img src="volunteer.jpeg" alt="Image" class="img-fluid" >
          </div>
        </div>            
    <div class="wrapper">
       <font color="black"> <h4>APPLY AS A VOLUNTEER</h4>
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST" enctype="multi/form-data">
            ...
            <div class="form-group  <?php echo (!empty($nameErr)) ? 'has-error' : ''; ?>">
             ...
             <div class="form-group <?php echo (!empty($educationErr)) ? 'has-error' : ''; ?>">
                <label>EDUCATION (QUALIFICATION)</label>
                <input type="text" name="education" class="form-control" value="<?php echo $education; ?>">
                <span class="help-block"><?php echo $educationErr; ?></span>
            </div>    
            ...
            <div class="form-group <?php echo (!empty($experienceErr)) ? 'has-error' : ''; ?> ">
                <label>ANY EXPERIENCE (WITH OTHER NGOS)</label>
                <font size="2" weight="50">IF YES. ENTER NGOS NAME </font>
                <input type="text" name="experience" class="form-control" value="<?php echo $experience; ?>">
                <span class="help-block"><?php echo $experienceErr; ?></span>
            </div>
            <div class="form-group">
                <input type="submit" onclick="myFunction()" class="btn btn-primary" value="APPLY NOW">
            </div>
        </font>
        </form>
    </div>  
